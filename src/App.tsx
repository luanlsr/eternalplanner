...
import { FinancialView } from './components/suppliers/FinancialView';
import { PlanningView } from './components/suppliers/PlanningView';
import { SupplierModal } from './components/suppliers/AddSupplierModal';
...
import { ChevronLeft, CheckCircle2, Circle, Calendar, Printer, Download, Share2, Heart, DollarSign, FileText, Edit2, Clock, AlertTriangle } from 'lucide-react';
import { cn } from './components/ui';
import { parseISO, differenceInDays } from "date-fns";

function App() {
  const [activeTab, setActiveTab] = useState('dashboard');
  const { data, addSupplier, updateSupplier, deleteSupplier, updateInstallment, updateConfig, updateWeddingInfo, reorderSuppliers } = useWeddingData();
    ...
    if (newStatus === 'pago') {
      ...
      if (supplier) {
        ...
        if (remaining.length === 0) {
           // Celebration Sound (Fantasy Success)
  ...
  return (
    <div className={cn("min-h-screen transition-colors duration-500", isDark ? "dark bg-background text-foreground" : "bg-slate-50 text-slate-900")}>
      <Sidebar 
        activeTab={activeTab} 
